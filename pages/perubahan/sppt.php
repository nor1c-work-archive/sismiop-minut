...
if (isset($_POST['cari']) || isset($_GET['cari'])) {
    ...
    case 'nop':
        if (isset($_POST['KD_PROPINSI']) AND isset($_POST['KD_DATI2']) AND isset($_POST['KD_KECAMATAN']) AND isset($_POST['KD_KELURAHAN']) AND isset($_POST['KD_BLOK']) AND isset($_POST['NO_URUT']) AND isset($_POST['KD_JNS_OP'])) {
          ...
          if ($KD_PROPINSI == "" || $KD_DATI2 == "" || $KD_KECAMATAN == "" || $KD_KELURAHAN == "" || $KD_BLOK == "" || $NO_URUT == "" || $KD_JNS_OP == "") {
            ...
            $q = "SELECT count(*) from ALL_SPPT where
                  THN_PAJAK_SPPT = '".date('Y', time())."' and
  ...
  $q = "SELECT count(*) FROM ALL_SPPT where THN_PAJAK_SPPT='".date('Y', time())."'";
}
...
<!-- BATAS MENU -->
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Perubahan SPPT Tahun Berjalan</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
            ...
            <div class="row">
                    ...
                    <div class="panel panel-primary">
                        <div class="panel-heading">
                            Home &gt; Perubahan &gt; Perubahan SPPT Tahun Berjalan
                        </div>
                        <!-- /.panel-heading -->
    ...
    if (isset($_GET['cari']) || isset($_POST['cari'])) {
      if ($numrow=="0") {
          ...
          switch ($opt) {
            case 'nop':
              ...
              break;
            case 'nmwp':
                $q = "SELECT * FROM ALL_SPPT WHERE THN_PAJAK_SPPT='".date('Y', time())."' and NM_WP_SPPT LIKE '%".$keyword."%' LIMIT $limitmin, $limitmax
                ";
              break;
            default:
              break;
      ...
      $q = "SELECT * FROM ALL_SPPT WHERE THN_PAJAK_SPPT='".date('Y', time())."' LIMIT $limitmin, $limitmax";
    }
    ...
    <tbody>
        <tr>
            ...
            <td><?=$row['JLN_WP_SPPT']?></td>
            <td><?=$row['KD_KELURAHAN']?></td>
            <td><?=$row['RT_WP_SPPT'].'/'.$row['RW_WP_SPPT']?></td>
            <td><?=$row['NM_WP_SPPT']?></td>
            <td><?=$row['THN_PAJAK_SPPT']?></td>
            <td>
              <a href="edit-sppt.php?refid=<?=$row['KD_PROPINSI'].'.'.$row['KD_DATI2'].'.'.$row['KD_KECAMATAN'].'.'.$row['KD_KELURAHAN'].'.'.$row['KD_BLOK'].'.'.$row['NO_URUT'].'.'.$row['KD_JNS_OP']?>" class="btn btn-success btn-sm">Edit</a>
            </td>
        </tr>
